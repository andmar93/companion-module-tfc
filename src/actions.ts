	...
	self.setActionDefinitions({
		...
		routeByUUID: {
			...
			callback: async (event, context) => {
				...
				const selectedSource = (await context.parseVariablesInString(event.options.sourceUUID as string)).trim()
				const selectedTarget = (await context.parseVariablesInString(event.options.targetUUID as string)).trim()
